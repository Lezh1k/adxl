...
//            inclk = (inclk * 256) / (uint64_t) (256 + mult);
            ...
            if(inclk < 0xFFFFFF){
                uint32_t smc = inclk;
                inclk = smc*256 / (256+mult);
            }else{
                uint64_t div = (256 + mult);
                uint64_t div8b = (256 + mult)*10;
                uint64_t div16b = (256 + mult)*65536;
                uint64_t div22b = (256u + mult)<<22u;
                uint64_t del = (inclk * 256);
                uint32_t rezult = 0;
                while(del >= div22b){
                    del -= div22b;
                    rezult += 1<<22;
